...
import "../interfaces/IBatasmStaking.sol";

/*  BatasmTreasury.sol ============================================
    The contract is to store the reserve of Batasm Protocol
...
==================================================================== */
contract BatasmTreasury is Ownable {
    using SafeERC20 for IERC20;
    using Address for address;

    mapping(address => bool) public strategies;
    address[] public strategiesArray;
    IBatasmStaking public staking;

    constructor(IBatasmStaking _staking) {
        staking = _staking;
    }

    // ========== PUBLIC VIEW FUNCTIONS ============

    /// @notice Return ERC-20 balance of BatasmTreasury
    /// @param _token Address of the ERC-20 token
    /// @return Balance of the BatasmTreasury
    ...
    function requestFund(address _token, uint256 _amount) external {
        require(strategies[msg.sender], "BatasmTreasury::requestFund: Only strategies can request fund");
        require(_amount <= balanceOf(_token), "BatasmTreasury::requestFund: Request more fund than balance");
    ...
    function addStrategy(address _strategy) external onlyOwner {
        require(_strategy != address(0), "BatasmTreasury::addStrategy: invalid address");
        require(!strategies[_strategy], "BatasmTreasury::addStrategy: strategy was previously added");
    ...
    function removeStrategy(address _strategy) external onlyOwner {
        require(strategies[_strategy], "BatasmTreasury::removeStrategy: strategy not found");
    ...
    function allocateFee(address _token, uint256 _amount) external onlyOwner {
        require(address(staking) != address(0), "BatasmTreasury::allocateFee:Fee distributor not set");
        require(_amount > 0, "BatasmTreasury::allocateFee: invalid amount");
